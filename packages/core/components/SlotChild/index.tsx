...
}: // path,
{
  ...
  const userIsDragging = false; //!!ctx?.draggedItem;
  ...
  const { ref: sortableRef, status } = useSortableSafe<ComponentDndData>({
    ...
    type: "component",
    data: {
      areaId: "", //path[path.length - 1] as string,
      zone: zoneCompound,
      index,
      componentType,
      containsActiveZone,
  ...
  const indicativeHover = false; //ctx?.hoveringComponent === id;
