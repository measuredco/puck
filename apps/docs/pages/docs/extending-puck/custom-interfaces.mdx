...
Custom interfaces can be implemented by providing [`children`](/docs/api-reference/components/puck#children) to the [`<Puck>` component](/docs/api-reference/components/puck):
...
Access the Puck [`AppState`](/docs/api-reference/data-model/app-state) via the [`usePuck`](/docs/api-reference/functions/use-puck) hook to integrate with Puck with custom editor components:
