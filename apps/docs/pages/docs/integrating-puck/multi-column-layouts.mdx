...
Apply the [CSS display](https://developer.mozilla.org/en-US/docs/Web/CSS/display) property to a DropZone to arrange your components in different layouts. Puck supports all `display` values, including `grid` and `flex`, for multi-directional drag-and-drop.
