...
```tsx {2-6,15} showLineNumbers copy
const config = {
  categories: {
    typography: {
      components: ["Card"],
...
Use slots with [`defaultProps`](/docs/api-reference/configuration/component-config#defaultprops) to pre-populate it when the component is inserted.

```tsx {9-19} showLineNumbers copy
const config = {
  components: {
    Example: {
      fields: {
