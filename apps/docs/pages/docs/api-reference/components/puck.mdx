...
| [`initialContent`](#initialContent) | `'<!DOCTYPE html><html><head><style>body { background: hotpink; }</style></head><body><div class="frame-root"></div></body></html>'` | string  | -      |
...
Allows injection and styling of the initial iframe component. Defaults to `undefined`

In order for Puck to load as required, the `<body>` tag must contain the following `<div>`

```html
<div class="frame-root"></div>
```

iframes need to be enabled in order for this to load and `enabled: true` flag must be set.
