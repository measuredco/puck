...
- This field produces an array of [ComponentData](/docs/api-reference/component-data)
...
<PuckPreview
  label="Example"
  renderDrawer={() => {
    return <CodeBlockDrawer getCode={(appState) => appState.data.root.props} />;
  }}
  config={{
    root: {
      fields: {
