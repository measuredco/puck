...
The main configuration for Puck, describing the available components, fields and more.
...
| [`root`](#root)             | `root: { render: () => <div /> }`                              | [`ComponentConfig`](/docs/api-reference/component-config) | -        |
...
An object describing the components available to Puck. Each component definition implements the [`ComponentConfig` API](/docs/api-reference/component-config).
...
An object describing the root of your component tree. This component wraps the rest of your components. Implements the [`ComponentConfig` API](/docs/api-reference/component-config).
...
The user-facing label for the category. Will use the `category` key if not provided.
